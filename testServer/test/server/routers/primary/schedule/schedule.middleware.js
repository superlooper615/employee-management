...
class ScheduleMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./schedule.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./schedule.policy').acl);
  }


  async default (req, res, next) {

    console.log('Schedule middleware;')
