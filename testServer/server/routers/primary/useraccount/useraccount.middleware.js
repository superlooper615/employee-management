...
class UseraccountMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./useraccount.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./useraccount.policy').acl);
  }


  async default (req, res, next) {

    console.log('Useraccount middleware;')
